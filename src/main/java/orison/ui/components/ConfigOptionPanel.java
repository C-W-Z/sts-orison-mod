...
public class ConfigOptionPanel implements ConfigUIElement {
    ...
    protected static float LINE_SPACING = Settings.BIG_TEXT_MODE ? (40.0F * Settings.scale) : (32.0F * Settings.scale);

    public List<ConfigUIElement> scrollables;
    protected float realHeight;
    protected float nextYPos;

    protected float x; // 左側x座標
    protected float rightX; // 右側x座標
    protected float y; // 底部y座標
